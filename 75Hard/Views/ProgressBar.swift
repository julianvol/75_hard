        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                RoundedRectangle(cornerRadius: 12.5)
                    ...
                    .frame(width: 26+(min(CGFloat(max(min(todaysChallengeDayIndex,74),0))/74.0, 1)*(geometry.size.width-26)), height: 26)
                
                RoundedRectangle(cornerRadius: 8)
                    .foregroundColor(Color("selection"))//Color(red: 224/255.0, green: 161/255.0, blue: 79/255.0)
                    .frame(width: 16, height: 16)
                    .padding(.leading, 4.5+min(CGFloat(selectedChallengeDayIndex)/74.0, 1) * (geometry.size.width-16-9))
